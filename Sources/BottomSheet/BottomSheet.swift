...
public class Bottomsheet {
    ...
    open class Controller: UIViewController {
        ...
        // Adds ContentsView
        open func addContainer<T>(with viewController: T, configurationHandler: ((T) -> Void)) {
            self.isScrollEnabledInSheet = false
            add(childVC: viewController as! UIViewController, to: containerView)
            configurationHandler(viewController)
        }
        ...
        open func addCustomScrollView(scrollView: UIScrollView) {
            self.scrollView = scrollView
...
open class BottomsheetUtilities {
    public static func createBottomSheet<Destination,Source>(
        ...
        bottomSheet.initializeHeight = presentedWithHeight ?? 300
        guard let sourceVC = sourceVC as? UIViewController else {
            fatalError("Source not a type of UIViewController")
        }
        DispatchQueue.main.async {
            sourceVC.present(bottomSheet, animated: true, completion: nil)
        }
